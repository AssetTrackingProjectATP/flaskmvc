...
def get_assets():
    ...
    for asset in assets:
        ...
        if asset.get('assignee_id'):
            ...
            if assignee:
                # Use __str__ method which handles None lname
                asset['assignee_name'] = str(assignee)
            else:
                asset['assignee_name'] = f"Assignee ID: {asset['assignee_id']}" # More informative if lookup fails
        else:
...
def asset_report(asset_id):
    ...
    for event in scan_events:
        try:
             # Assuming ScanEvent has a get_json or similar method
            event_dict = {}
            if hasattr(event, 'get_json') and callable(event.get_json):
                 event_dict = event.get_json()
            else: # Fallback to __dict__ if no get_json
                event_dict_raw = event.__dict__.copy()
                # Clean _sa_instance_state if present
                event_dict_raw.pop('_sa_instance_state', None)
                # Convert datetime objects if needed (adjust formatting)
            ...
            enriched_scan_events.append({
                ...
                'scan_time: ': getattr(event, 'scan_time', datetime.now()).strftime('%Y-%m-%d %H:%M:%S') # Format date
            })

    # Sort by scan time, handling potential missing keys/different formats
    def get_scan_time(event_dict):
        ...
        if isinstance(time_val, str):
            ...
            return datetime.min # Fallback for other types or None
...
def add_asset_api():
    ...
    assignee_name = data.get('assignee_name') # Get name instead of id
    ...
    assignee_id = assignee.id # Get the ID of the found/created assignee
    # --- End Assignee Handling ---
    ...
    try:
        new_asset = add_asset(
            ...
            assignee_id=assignee_id, # Use the retrieved/created assignee ID
        ...
        if new_asset:
            ...
            if existing_asset:
                 ...
                 # Check if the room was invalid (add_asset returns None)
